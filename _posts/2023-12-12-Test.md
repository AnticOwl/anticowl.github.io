...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td>F2</td>
            <td>Disable the depth of field</td>
        </tr>
        <tr>
            <td>F3</td>
...
#### Image comparison with in-game setting at highest values and Ultra Grpahics.
...
Let say you want to take a shot of Faith jumping from one roof the another. You will need to use the `Dolly` option from `mmultiplayer` to accomplish this. Keep in mind that the count is made in frames and not in seconds. But here, it has not much importance. So, to do so, follow these steps:
